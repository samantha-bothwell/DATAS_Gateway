...
\[
\left[n\alpha - t_{\alpha/2}\sqrt{n\alpha(1-\alpha)}, n\alpha + t_{\alpha/2}\sqrt{n\alpha(1-\alpha)}\right].
\]
...
```{r}
qbinom(c(0.025,0.975), 100, 0.05)

50 - 1.96*6.89/sqrt(1000)
50 + 1.96*6.89/sqrt(1000)
```
So, the 95% confidence interval for the number of false positives is \textbf{[37, 64]} - or we can say the 95% confidence interval for the proportion of false positives is [0.037, 0.064].

Unfortunately, we cannot always know exactly where our false positive lies. But we are 95% confident that the true number of false positives present in a sample of 1000 is within the interval [37, 64].
...
Since the samples are taken from a standard normal distribution, we know our null is true. Let's see when we'll reject the null:
```{r}
set.seed(32)
...
```{r}
# Plot the CIs
plot(c(0,1), c(min(dat$Lower), max(dat$Upper)), type = "n",
     xlab = paste(n, "replicates of confidence intervals"), ylab = "", xaxt = "n")
x = (1:n)/n #index for each of the n CIs
...
Now let's run some simulations to find the distribution and confidence interval around the number of false positives. 
...
Now we can use this simulation to determine the confidence interval for the proportion of false positives - before we found the theoretical confidence interval to be [0.037, 0.064]. Let's see how close our simulation came to this:
```{r}
t.test(misses)
```
