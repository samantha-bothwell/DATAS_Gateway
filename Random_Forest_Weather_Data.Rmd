...
The tree first determines the most important variable in predicting the temperature. Here, it appears that month gives us a good indication about the weather. For the months June, July, August, and September the temperature is the highest (at least where the samples were taken - Fort Collins, CO). 

```{r}
IncNodePur = model$importance[1:13]
names = colnames(train)[c(1:4,6:14)]
...
p = ggplot(Imp, aes(IncNodePur, names)) +
  ...
  xlim(1000, 875000) + ylab("Predictor") + labs(title = "Variable Importance Plot") + 
...
```
For this model, the mean squared error is \textbf{4.76}
...
Now, it's intuitive that month has a strong impact on the temperature. Let's take time out of the data and see how the random forest algorithm performs:
```{r new data}
dat = dat[,-c(1:4)]
```
...
Let's evaluate how this model performed with the MSE:
```{r}
predictions = predict(model, test)

MSE(predictions, test$Temp_degF)
```
It turns out that our mean squared error is actually lower when we remove time. Why is this?
...
MSE(predictions, test$Temp_degF)
```
Our mean squared error in this case is significantly larger than the previous examples! 
